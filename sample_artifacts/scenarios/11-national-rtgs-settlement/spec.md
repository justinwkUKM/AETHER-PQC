...
### Node 1: Settlement Message Ingress Gateway
* **Label**: `Application`
...
* **Purpose**: Primary partner ingress gateway terminating settlement message queues and routing payment files from commercial bank members into the clearing core.

### Node 2: Wholesale Settlement Core
...
### Node 5: External Settlement Messaging Network
...
* **Purpose**: Connects a regional clearinghouse to international external settlement messaging services for cross-border currency exchanges.

---

## 3. Communication Link Relationships

1. **Settlement Message Ingress Gateway** (Application) connects to **Wholesale Settlement Core** (SoftwareComponent) via `DEPENDS_ON` link to transmit inbound interbank settlements.
2. **Wholesale Settlement Core** (SoftwareComponent) connects to **Government Securities Depository** (DataAsset) via `PROCESSES` connection to write bond ledger rows.
3. **Wholesale Settlement Core** (SoftwareComponent) connects to **Central Reserve Ledger** (DataAsset) via `PROCESSES` connection to perform debit/credit balances.
4. **Wholesale Settlement Core** (SoftwareComponent) connects to **External Settlement Messaging Network** (ExternalService) via `CALLS` connection for foreign currency routing.
